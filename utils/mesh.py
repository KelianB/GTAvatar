...
from utils.geometry import dot, safe_normalize, compute_vertices_face_normals, find_edges, find_connected_faces, compute_laplacian_uniform
...
class Mesh:
    ...
    def get_vertices_face_normals(self, vertices):
        ...
        Args:
            vertices (tensor): New vertex positions (Vx3)
        """
        vertex_normals, face_normals = compute_vertices_face_normals(vertices, self.indices)
    ...
    def laplacian(self):
        if self._laplacian is None:
            self._laplacian = compute_laplacian_uniform(self.vertices, self.edges)
