...
from .lbs import  vertices2landmarks, batch_rodrigues, flame_fn
...
# FLAME_MODEL_PATH = "assets/flame/flame2023.pkl" # FLAME 2023
FLAME_MODEL_PATH = "assets/flame/flame2020.pkl" # FLAME 2020
FLAME_PARTS_PATH = "assets/flame/FLAME_masks.pkl" # FLAME Vertex Masks

# Landmarks embeddings (unused)
LMK_EMBEDDING_PATH = "assets/flame/landmark_embedding_with_eyes.npy"
...
class FLAME(nn.Module):
    ...
    def __init__(
        ...
        include_mask=True,
        add_teeth=False,
        add_mouth_interior=False,
    ):
        super().__init__()
        self.fix_mouth_winding_order = True # fix triangle winding order for teeth and mouth interior
        ...
        shapedirs = torch.cat(
            [shapedirs[:, :, :n_shape_params], shapedirs[:, :, 300 : 300 + n_expr_params]],
            2,
        )
        self.register_buffer("shapedirs", shapedirs)

        # The pose components
        self.register_buffer("posedirs", to_tensor(to_np(flame_model.posedirs), dtype=self.dtype) * scale) # (V, 3, 36)
        ...
        # Load eyelids from SMIRK        
        self.register_buffer("l_eyelid", torch.from_numpy(np.load(f"assets/flame/l_eyelid.npy")).to(self.dtype).unsqueeze(0) * scale)
        self.register_buffer("r_eyelid", torch.from_numpy(np.load(f"assets/flame/r_eyelid.npy")).to(self.dtype).unsqueeze(0) * scale)
    ...
    def forward(
        ...
        shapedirs=None, posedirs=None, lbs_weights=None,
        l_eyelid_mask=None, r_eyelid_mask=None,
        J_regressor=None,
    ):
        shapedirs = self.shapedirs if shapedirs is None else shapedirs
        shapedirs_id = shapedirs[..., :self.n_shape_params]
        shapedirs_expr = shapedirs[..., self.n_shape_params:]
        posedirs = self.posedirs if posedirs is None else posedirs 
        lbs_weights = self.lbs_weights if lbs_weights is None else lbs_weights
        v_template = self.v_template if template_verts is None else template_verts
        l_eyelid = self.l_eyelid if l_eyelid_mask is None else l_eyelid_mask
        r_eyelid = self.r_eyelid if r_eyelid_mask is None else r_eyelid_mask
        J_regressor = self.J_regressor if J_regressor is None else J_regressor
       
        vertices, transform_dict = flame_fn(
            v_template,
            shape=shape,
            expression=expr,
            full_pose=pose,
            eyelids=eyelid_params,
            translation=translation,
            lbs_weights=lbs_weights,
            shapedirs_expr=shapedirs_expr,
            posedirs=posedirs,
            shapedirs_id=shapedirs_id,
            l_eyelid_dirs=l_eyelid,
            r_eyelid_dirs=r_eyelid,
            J_regressor=J_regressor,
            rot_parents=self.parents,
            zero_centered_at_root_node=zero_centered_at_root_node,
        )

        out_dict = transform_dict
        
        # compute landmarks if desired
        if return_landmarks:
            bz = vertices.shape[0]
            out_dict["landmarks"] = vertices2landmarks(
                vertices,
                self.faces,
        ...
        return vertices, out_dict

    def add_teeth(self):
        # teeth placement parameters
        dist_to_lips = 2.5
        lower_teeth_back_dist = 0.3
        thickness_scale = 1.0
        y_offset_upper = -0.6
        y_offset_lower = 0.7
        ...
        if False:       
            ...
            v_top = 1 - 0.008
            v_bottom = 1 - 0.0425
            v_step = abs(v_top - v_bottom) / 8
            u = torch.linspace(u_center + u_width/4, u_center - u_width/4, 15)
            v = torch.tensor([v_top, v_top-v_step*6, v_top-v_step*7, v_top-v_step*8])
            v = v[[0, 0, 1, 1, 3, 2, 3, 2]]
            ...
            v = v[[3, 2, 0, 1, 3, 4, 6, 5]] # lower teeth are not good
            uv = torch.stack(torch.meshgrid(u, v, indexing='ij'), dim=-1).permute(1, 0, 2).reshape(num_verts_teeth, 2)  # (#num_teeth, 2)

        num_verts_uv_orig = self.verts_uvs.shape[0]

        # shapedirs: copy identity shapes from lips, set expression shapes to zero
        shape_dirs_mean_id = (self.shapedirs[vid_lip_outside_ring_upper, :, :self.n_shape_params] + self.shapedirs[vid_lip_outside_ring_lower, :, :self.n_shape_params]) / 2
        shape_dirs_mean_expr = (self.shapedirs[vid_lip_outside_ring_upper, :, self.n_shape_params:] + self.shapedirs[vid_lip_outside_ring_lower, :, self.n_shape_params:]) / 2
        shape_dirs_mean = torch.cat((shape_dirs_mean_id, shape_dirs_mean_expr * 0), dim=-1)      
        self.shapedirs = torch.cat([self.shapedirs, torch.zeros_like(self.shapedirs[:num_verts_teeth])], dim=0)
        self.shapedirs[vid_teeth_upper_root] = shape_dirs_mean
        self.shapedirs[vid_teeth_lower_root] = shape_dirs_mean
        self.shapedirs[vid_teeth_upper_edge] = shape_dirs_mean
        self.shapedirs[vid_teeth_lower_edge] = shape_dirs_mean
        self.shapedirs[vid_teeth_upper_root_back] = shape_dirs_mean
        self.shapedirs[vid_teeth_upper_edge_back] = shape_dirs_mean
        self.shapedirs[vid_teeth_lower_root_back] = shape_dirs_mean
        self.shapedirs[vid_teeth_lower_edge_back] = shape_dirs_mean

        # posedirs set to zero
        self.posedirs = torch.cat([self.posedirs, torch.zeros_like(self.posedirs[:num_verts_teeth])], dim=0)
        ...
        self.l_eyelid = torch.cat([self.l_eyelid, torch.zeros_like(self.l_eyelid[:, :num_verts_teeth])], dim=1)  # (1, V, 3) -> (1, V+num_verts_teeth, 3)
        self.r_eyelid = torch.cat([self.r_eyelid, torch.zeros_like(self.r_eyelid[:, :num_verts_teeth])], dim=1)  # (1, V, 3) -> (1, V+num_verts_teeth, 3)
        ...
        if self.fix_mouth_winding_order:
            # Fix the winding order of faces so all normals point forward
            fix_winding_orders(v_teeth, f_teeth_upper, y_dir="down")
            fix_winding_orders(v_teeth, f_teeth_lower, y_dir="up")

        if False:
            # shift the upper teeth to the left, lower teeth to the right
            uv[vid_teeth_upper - num_verts_orig, 0] -= u_width / 4
            uv[vid_teeth_lower - num_verts_orig, 0] += u_width / 4
    ...
    def add_mouth_interior(self):
        ...
        v_interior_front1=v_teeth_lower_root.clone()
        v_interior_front1[:,2]-=mean_dist*2
        v_interior_front2=v_teeth_lower_root.clone()
        v_interior_front2[:,2]-=mean_dist*3
        v_interior_front3=v_teeth_lower_root.clone()
        v_interior_front3[:,2]-=mean_dist*4
        v_interior_front4=v_teeth_lower_root.clone()
        v_interior_front4[:,2]-=mean_dist*5
        
        v_interior_upper1=v_interior_front4.clone()
        v_interior_upper1[:,1]+=mean_dist*1
        v_interior_upper2=v_interior_front4.clone()
        v_interior_upper2[:,1]+=mean_dist*2
        v_interior_upper3=v_interior_front4.clone()
        v_interior_upper3[:,1]+=mean_dist*3
        v_interior_upper4=v_interior_front4.clone()
        v_interior_upper4[:,1]+=mean_dist*4
        v_interior_upper5=v_interior_front4.clone()
        v_interior_upper5[:,1]+=mean_dist*5
        
        num_verts_orig = self.v_template.shape[0]
        num_verts_uv_orig = self.verts_uvs.shape[0]

        vid_interior_front1 = torch.arange(0, 15) + num_verts_orig
        vid_interior_front2 = torch.arange(15, 30) + num_verts_orig
        vid_interior_front3 = torch.arange(30, 45) + num_verts_orig
        vid_interior_front4 = torch.arange(45, 60) + num_verts_orig
        vid_interior_upper1=torch.arange(60, 75) + num_verts_orig
        vid_interior_upper2=torch.arange(75, 90) + num_verts_orig
        vid_interior_upper3=torch.arange(90, 105) + num_verts_orig
        vid_interior_upper4=torch.arange(105, 120) + num_verts_orig
        vid_interior_upper5=torch.arange(120, 135) + num_verts_orig

        v_interior=torch.cat([
            v_interior_front1,v_interior_front2,v_interior_front3,v_interior_front4,  # num_verts_orig + 0-59 
            v_interior_upper1,v_interior_upper2,v_interior_upper3,v_interior_upper4,v_interior_upper5,# num_verts_orig + 60-134
        ], dim=0)
        self.v_template = torch.cat([self.v_template, v_interior], dim=0)
        
        num_ring,num_vertex=9,v_interior_front1.shape[0]
        num_interior_vertex=v_interior.shape[0]
        f_interior=torch.zeros([(num_ring-1)*(num_vertex-1)*2,3])
        
        for i in range(num_ring-1):
            for ver_idx in range(num_vertex):
                if ver_idx == num_vertex-1:
                    pass
                else:
                    f_interior[i*num_vertex*2+ver_idx*2-i*2,:]=torch.tensor([ver_idx+i*num_vertex,ver_idx+(i+1)*num_vertex,ver_idx+(i+1)*num_vertex+1])
                    f_interior[i*num_vertex*2+1+ver_idx*2-i*2,:]=torch.tensor([ver_idx+i*num_vertex,ver_idx+i*num_vertex+1,ver_idx+(i+1)*num_vertex+1])
        f_interior=f_interior.to(torch.long)

        if self.fix_mouth_winding_order:
            # Fix the winding order of faces so all normals point forward
            fix_winding_orders(v_interior, f_interior, y_dir="up")

        self.faces = torch.cat([self.faces, f_interior+num_verts_orig], dim=0).to(torch.long)       
        ...
        if False:
            ...
            v_top = 1 - 0.008
            v_bottom = 1 - 0.0425
            u = torch.linspace(u_center + u_width/2, u_center - u_width/2, num_vertex)
            v = torch.linspace(v_bottom, v_top, num_ring)
            uv = torch.stack(torch.meshgrid(u, v, indexing='ij'), dim=-1).permute(1, 0, 2).reshape(num_interior_vertex, 2)
            num_verts_uv_orig = self.verts_uvs.shape[0]
            self.verts_uvs = torch.cat([self.verts_uvs, uv], dim=0)
            self.textures_idx = torch.cat([self.textures_idx, f_interior+num_verts_uv_orig], dim=0)
        else:
            self.verts_uvs = torch.cat([self.verts_uvs, torch.zeros((num_interior_vertex,2))], dim=0)
            self.textures_idx = torch.cat([self.textures_idx, f_interior+num_verts_uv_orig], dim=0)
        ...
        shape_dirs_mean_expr = (self.shapedirs[vid_lip_outside_ring_upper, :, self.n_shape_params:] + self.shapedirs[vid_lip_outside_ring_lower, :, self.n_shape_params:]) / 2
        shape_dirs_mean = torch.cat((shape_dirs_mean_id, shape_dirs_mean_expr * 0), dim=-1)
        self.shapedirs = torch.cat([self.shapedirs, torch.zeros_like(self.shapedirs[:num_interior_vertex])], dim=0)
        self.shapedirs[vid_interior_front1] = shape_dirs_mean
        self.shapedirs[vid_interior_front2] = shape_dirs_mean
        self.shapedirs[vid_interior_front3] = shape_dirs_mean
        self.shapedirs[vid_interior_front4] = shape_dirs_mean
        self.shapedirs[vid_interior_upper1] = shape_dirs_mean
        self.shapedirs[vid_interior_upper2] = shape_dirs_mean
        self.shapedirs[vid_interior_upper3] = shape_dirs_mean
        self.shapedirs[vid_interior_upper4] = shape_dirs_mean
        self.shapedirs[vid_interior_upper5] = shape_dirs_mean
        
        # posedirs set to zero
        self.posedirs = torch.cat([self.posedirs, torch.zeros_like(self.posedirs[:num_interior_vertex, :, :])], dim=0)
        
        # J_regressor set to zero
        self.J_regressor = torch.cat([self.J_regressor, torch.zeros_like(self.J_regressor[:, :num_interior_vertex])], dim=1)  # (5, J) -> (5, J+num_verts_teeth)

        # lbs_weights manually set
        self.lbs_weights = torch.cat([self.lbs_weights, torch.zeros_like(self.lbs_weights[:num_interior_vertex])], dim=0)  # (V, 5) -> (V+num_verts_teeth, 5)
        self.lbs_weights[vid_interior_front1, 2] += 1  # move with jaw
        self.lbs_weights[vid_interior_front2, 2] += 1  # move with jaw
        self.lbs_weights[vid_interior_front3, 2] += 1  # move with jaw
        self.lbs_weights[vid_interior_front4, 2] += 1  # move with jaw
        self.lbs_weights[vid_interior_upper1, 1] += 1  # move with neck
        self.lbs_weights[vid_interior_upper2, 1] += 1  # move with neck
        self.lbs_weights[vid_interior_upper3, 1] += 1  # move with neck
        self.lbs_weights[vid_interior_upper4, 1] += 1  # move with neck
        self.lbs_weights[vid_interior_upper5, 1] += 1  # move with neck
        
        self.l_eyelid = torch.cat([self.l_eyelid, torch.zeros_like(self.l_eyelid[:,:num_interior_vertex,:])],dim=1)
        self.r_eyelid = torch.cat([self.r_eyelid, torch.zeros_like(self.r_eyelid[:,:num_interior_vertex,:])],dim=1)
        ...
        """
            ...
            Input:
                vertices: torch.tensor NxVx3, dtype = torch.float32
                    The tensor of input vertices
                full_pose: torch.tensor N X 15, dtype = torch.float32
...
class FlameMask(nn.Module):
    ...
    def process_face_mask(self, faces):
        ...
        # float masks
        self.fl = BufferContainer()
        for region_name, v_mask in self.v.items():
            fl_mask = torch.zeros((self.num_verts, 1), dtype=torch.float)
            fl_mask[v_mask] = 1.0
            self.fl.register_buffer(region_name, fl_mask)
...
def fix_winding_orders(verts, faces, y_dir=None):
    # Fix the winding order of faces so all normals point forward
    va, vb, vc = verts[faces].unbind(-2)
    normals = (vb-va).cross(vc-va, dim=-1) # (F, 3)
    y = normals[:, 1] # (F)
    z = normals[:, 2] # (F)

    # note: x[[1,2]] = x[[2,1]] is not supported by pytorch, we have to clone
    mask = z < 0
    faces[mask] = faces[mask].clone()[:,[0,2,1]]

    if y_dir is not None:
        # For perfectly horizontal faces, fix the y direction
        mask = torch.logical_and(z == 0, (y < 0) if y_dir == "up" else (y > 0))
        faces[mask] = faces[mask].clone()[:,[0,2,1]]
